...
def get_all_versions() -> typing.List[Version]:
    response = urllib3.request("GET", "https://pypi.org/pypi/pyright/json")
...
def get_current_version(pyproject: dict) -> Version:
    ...
    requirement = next((r for r in requirements if r.name == "pyright"), None)
    assert requirement is not None, "pyproject.toml does not have pyright requirement"

    specifiers = list(requirement.specifier)
    assert (
        len(specifiers) == 1 and specifiers[0].operator == "=="
    ), f"pyright's specifier should be exact matching, but `{requirement}`"
...
def process_version(version: Version) -> typing.Sequence[str]:
    def replace_pyproject_toml(content: str) -> str:
        return re.sub(r'"pyright==.*"', f'"pyright=={version}"', content)

    def replace_readme_md(content: str) -> str:
        content = re.sub(r"rev: v\d+\.\d+\.\d+", f"rev: v{version}", content)
        return re.sub(r"/pyright/\d+\.\d+\.\d+\.svg", f"/pyright/{version}.svg", content)
